...
## Keyword product search (buyer search + add-listing)

With 152 SKUs, forcing every buyer/supplier through Category → Product was slow if you didn't already know which of the 9 categories held what you wanted. Both `/search` (`BuyerSearch.tsx`) and `/dashboard/add-listing` (`AddListingForm.tsx`) now have a **Search products** text input above the Category dropdown.

- **Client-side filter, not a new query** — both pages already fetch the full product list once on page load; typing filters that in-memory list against `product_name` and `product_name_am` (`matchesProductQuery()` in `src/lib/products/displayName.ts`), so results update on every keystroke with no round-trip.
- **Search spans every category** — a non-empty query shows matches regardless of category; picking one auto-syncs the Category dropdown behind it (disabled while searching, since it'd otherwise conflict with what's typed). Clearing the box goes back to normal category browsing.
- **Buyer search still can't "search then browse the category anyway"** — `search_listings` only understands `category` + `product_id`, not free text, so picking a specific product from search results is required before hitting Search (the button is disabled otherwise) rather than silently searching the wrong category.
- **Category labels are now readable** — `formatCategoryLabel()` turns `aggregates_blocks` into "Aggregates blocks" instead of showing the raw DB slug, used consistently by both dropdowns.
